...
def set_env_vars(env_vars:Dict[str, Tuple[Optional[str], Optional[str]]], raise_exec=False):
    """
    Check if env vars are already set, if not then set from the dict if non-None.
    If value in dict is None and if raise_exec is True, then raise exception else ignore.

    Parameters:
    - env_vars (Dict[str, Tuple[Optional[str], Optional[str]]]): name: (value, exception_message) for each env var.
    - raise_exec (bool): If True, raise exception if env var is not set, else ignore.
    """
